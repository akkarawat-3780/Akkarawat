...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) {
      showPopup('✅ สมัครสมาชิกสำเร็จ! กำลังเข้าสู่ระบบ...', 'success');

      // ✅ login อัตโนมัติ
      const loginRes = await fetch('/api/login/nisit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: form.email,
      ...
      if (loginRes.ok) {
        ...
        // ✅ เก็บ cookie สำหรับแสดงผลในระบบ
        document.cookie = `email=${encodeURIComponent(data.email)}; path=/`;
        document.cookie = `role=nisit; path=/`;
        document.cookie = `profile=${encodeURIComponent(data.profile || '/default-profile.png')}; path=/`;

        setTimeout(() => {
          window.location.href = '/nisit'; // 👉 เปลี่ยนเส้นทางไปหน้า dashboard นิสิต
        }, 2000);
      } else {
        showPopup('⚠️ สมัครสำเร็จ แต่เข้าสู่ระบบไม่สำเร็จ', 'error');
      }
