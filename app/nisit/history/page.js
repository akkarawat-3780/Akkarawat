...
export default function BorrowHistoryPage() {
  ...
  const [successPopup, setSuccessPopup] = useState({ show: false, message: "" }); // ✅ เพิ่ม popup สำเร็จ
  ...
  const confirmAction = async () => {
    ...
    if (modal.type === "cancel") {
      ...
      if (res.ok) {
        ...
        showPopup('❌ ไม่สามารถยกเลิกได้', 'error');
      }
    }

    if (modal.type === "lost") {
      const res = await fetch(`/api/borrow/${modal.id}/lost`, { method: 'POST' });
      if (res.ok) {
        showSuccessPopup('🚨 แจ้งหายเรียบร้อย (รอตรวจสอบ)');
        fetchHistory();
      } else {
        showPopup('❌ ไม่สามารถแจ้งหายได้', 'error');
      }
    }
